	...
	$.fn.fancyTable = function(options) {
		...
		this.tableSort = function (elm) {
			if(typeof elm.fancyTable.sortColumn !== "undefined" && elm.fancyTable.sortColumn < elm.fancyTable.nColumns){
				...
				let rows = $(elm).find("tbody tr").toArray().sort(
					function(a, b) {
						let elma = $(a).find("td").eq(elm.fancyTable.sortColumn);
						let elmb = $(b).find("td").eq(elm.fancyTable.sortColumn);
						let cmpa = $(elma).data("sortvalue") ? $(elma).data("sortvalue") : elma.html();
						let cmpb = $(elmb).data("sortvalue") ? $(elmb).data("sortvalue") : elmb.html();
		...
		this.each(function() {
			...
			if(settings.searchable){
				let searchHeader = $("<tr>").addClass("fancySearchRow");
